				...
				inline char const* time_now_string()
				{
                    /*try
                    {
                        ...
                        if (oss.str().empty())
                        {
                            ...
                            auto lo = std::locale::classic();
                            auto on = std::locale(lo, _facet);
                            oss.imbue(on);
                        }
                        oss.str("");
                    ...
                    auto t = time(NULL); //获取目前秒时间  
                    auto local = localtime(&t); //转为本地时间  
                    strftime(buf, 200, "%Y-%m-%d %H:%M:%S", local);
...
#define INIT_LOGGER(logfile) do \
	{ \
		ns::util::log::auto_logger_file& file = ns::util::log::aux::writer_single<ns::util::log::auto_logger_file>(); \
...
#if 0
#define LOG_DBG ns::util::log::empty_logger()
#define LOG_INFO ns::util::log::empty_logger()
#define LOG_WARN ns::util::log::empty_logger()
#define LOG_ERR ns::util::log::empty_logger()
#define LOG_OUT ns::util::log::empty_logger()
#define LOG_FILE ns::util::log::empty_logger()
#else
#define LOG_DBG ns::util::log::logger(ns::util::log::LOGGER_DEBUG_STR)<<"["<<__FILE__<<"]"<<"["<<__LINE__<<"]"
#define LOG_INFO ns::util::log::logger(ns::util::log::LOGGER_INFO_STR)<<"["<<__FILE__<<"]"<<"["<<__LINE__<<"]"
#define LOG_WARN ns::util::log::logger(ns::util::log::LOGGER_WARN_STR)<<"["<<__FILE__<<"]"<<"["<<__LINE__<<"]"
